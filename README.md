...
| `pr-review` | Review repository fit before implementation quality, with a continue-or-exit gate when nothing changed since the previous review. Starts or approves PR CI on both first and follow-up passes, waits afterwards, and includes CI problems as blocking findings. Triage each finding before posting a `COMMENT` review. Follow-up runs check earlier feedback rather than repeating the full review. After posting LGTM, shows current merge blockers and asks separately whether to squash-merge |
