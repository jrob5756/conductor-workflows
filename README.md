...
| `pr-review` | Review a pull request and post the findings you approve: cuts a throwaway worktree, runs a concept review that gates the expensive work behind a human decision, runs a code review that reports without touching anything, then puts every finding to you individually to post as-is, promote or demote, reword or drop. Publishes one `COMMENT` review with inline comments anchored wherever the diff allows. Run it again on the same pull request and it checks what became of each point you made last time instead, closing on a choice between approving, re-raising what is still open, or a note you write yourself |
