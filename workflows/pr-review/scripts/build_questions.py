...
The three triage outcomes come from two choices plus the free-text path: a
third choice reading "reword it" would record that label and no wording, since
choices carry no follow-up prompt.
...
ANSWER_GUIDANCE = (
    f"Choose '{POST_AS_IS}' to post this wording, '{DO_NOT_POST}' to drop it, "
    "or write your own note to post it with your changes applied."
)
...
def question_for(finding: dict[str, object]) -> dict[str, object]:
    ...
    hint_parts.append(ANSWER_GUIDANCE)

    return {
        "id": finding["id"],
        "text": f"{finding['severity']}: {finding['title']}{suffix}",
        "hint": "\n\n".join(hint_parts),
        "choices": [POST_AS_IS, DO_NOT_POST],
