#!/usr/bin/env python3
"""triage_heuristic.py — Classify PR's changed files into specialist buckets.
...
specialists. The follow-up triage_llm step refines this; the resolver combines
both with the user's enabled_specialists/force_specialists.

Usage:
    triage_heuristic.py <pr_data_dir>
...
Exit codes:
    ...
    2  changed-files.json missing
    3  changed-files.json has unexpected shape
"""
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("pr_data_dir")
    try:
        args = parser.parse_args(argv)
    except SystemExit:
        print(
            json.dumps({"error": "triage_heuristic.py requires <pr_data_dir>"}),
            file=sys.stderr,
        )
        return 1

    pr_data_dir = Path(args.pr_data_dir)
    changed_path = pr_data_dir / "changed-files.json"

    if not changed_path.is_file():
        print(
            json.dumps(
                {"error": f"changed-files.json not found at {changed_path}"}
            ),
            file=sys.stderr,
        )
        return 2
    ...
    if isinstance(raw, dict):
        ...
        print(
            json.dumps({"error": "changed-files.json has unexpected shape"}),
            file=sys.stderr,
        )
        return 3
    ...
    for entry in files:
        ...
        if status == "removed":
            # Removed files don't need specialist review — only the absence
            # matters, which the lens reviewers will surface from the diff.
            continue
        active_files += 1
    ...
    if trivial_docs_only:
        ...
        if buckets["source"] or buckets["security_sensitive"] or buckets["config"]:
            ...
            if buckets["source"]:
                rationale_parts.append(
                    f"Source changes in {len(buckets['source'])} file(s) → code."
                )
        ...
        if buckets["types_eligible"]:
            ...
            rationale_parts.append(
                f"TypeScript files changed ({len(buckets['types_eligible'])}) → types."
            )

        # Security: lockfiles / dockerfiles / workflows / auth-pathed source / new
        # public endpoints. The heuristic is conservative — the LLM step refines
        # further (e.g. it spots "this Python diff adds an unauthenticated endpoint").
