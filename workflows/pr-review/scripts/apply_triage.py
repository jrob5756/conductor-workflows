...
Answers join to findings on `id`. The three outcomes are the two choices plus
the free-text path, so anything that is not one of the two known choices is
...
Output:
    ...
    recommended_count, anchored_count
"""
...
# These two must stay identical to the choices `build_questions.py` offers and
...
def main() -> None:
    ...
    for finding in findings:
        ...
        if normalized == DO_NOT_POST.casefold():
            ...
            continue
        entry = dict(finding)
        # Free text is the reword path, so anything that is not the "as-is"
        # choice is an instruction about how to word this finding.
