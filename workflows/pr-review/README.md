...
This workflow runs **5 deep-domain specialists** (general code, tests, error
handling, type design, comments) in parallel against a GitHub PR diff,
consolidates their findings, then deliberates the consolidated set through
**2 cross-cutting lens reviewers** (Security & Correctness vs Robustness &
Completeness) until they reach consensus or exhaust the round budget. Finally,
a **non-blocking polish stage** (code-simplifier + dead-code-finder) surfaces
...
specialist_gate (phantom-reviewer detection)
  ↓
consolidator (normalise native severities, dedupe ±5 lines, rank)
  ↓
lens_review (parallel)
  ├─ lens_security_correctness    (claude-opus-4.7 default)
  └─ lens_robustness_completeness (gpt-5.5 default — cross-family)
  ↓
deliberation rounds (sequential loop, max 1-10)
  ├─ deliberation_lens_1 → deliberation_lens_2 → arbitrator
  └─ arbitrator: another round, or move on
  ↓
polish_review (parallel — non-blocking, advisory only)
  ├─ polish_simplifier   (clarity / refactor opportunities)
  └─ polish_dead_code    (unused / unreachable code, with verification)
  ↓
report_writer → inline_comments_writer → output_writer
  ↓
[optional] poster (gh CLI — gated on auto_post=true)
  ↓
$end
```
...
- **`python3`** on PATH (used by the fetcher / poster scripts).
- **Node.js 18+** (used by the `@modelcontextprotocol/server-filesystem` MCP
  server).
- **Conductor v0.1.12+** (script agents and `reasoning.effort` forwarding).
...
| `focus` | string | `""` | Free-text guidance to direct reviewer attention (e.g., `"security and error handling"`) |
| `exclude` | array | `[]` | Aspects to de-prioritise: `correctness`, `security`, `error_handling`, `performance`, `testing`, `completeness` |
| `enabled_specialists` | array | `[code, tests, errors, types, comments]` | Subset of specialists to run |
| `include_polish` | bool | `true` | Run the polish_review stage (simplifier + dead-code-finder) |
| `polish_as_inline` | bool | `false` | Promote polish findings to inline PR comments (default: report-only) |
...
| `lens_security_model` | string | `claude-opus-4.7` | Model for the Security & Correctness lens |
| `lens_robustness_model` | string | `gpt-5.5` | Model for the Robustness & Completeness lens |
| `specialist_*_model` | string | various | Per-specialist model overrides |
...
conductor run workflows/pr-review/workflow.yaml \
  --input pr_url="https://github.com/owner/repo/pull/123" \
  --input lens_security_model="claude-opus-4.7-1m-internal"
```
...
arbitrator-findings.json   # consensus findings written by the arbitrator
consolidated-findings.json # the deduped set the lens reviewers deliberated over
```
...
polish agents + writers) runs ~14 LLM calls. To reduce cost:

- **Trim `enabled_specialists`** to the ones you need. `code` + `errors` is
...
The arbitrator emits a `confidence_score` in `[0, 100]` per:
...
- **Codebase context, custom rules, and work items** must be precomputed
  by the caller (e.g., a wrapper skill or CI script) and passed via path
  inputs. This workflow does not query a codebase index or fetch linked
  issues itself.
